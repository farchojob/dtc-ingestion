...
About seven and a half hours, in this order: one hour profiling the fixtures and writing the plan and the decisions before any code; two and a half on ingestion, staging and the migration; one and a half on marts, checks and the replay bugs above; one on tests; one on the console; the rest on this write-up. The console was time-boxed to an hour and stayed inside it because it only reads. I stopped with the marketing side thin on purpose: the brief said depth beats breadth, and the replay bugs were worth more than a ROAS column.
