...
Taken when the console got its design pass. Chosen: fourteen CSS custom properties (two surfaces, three inks, two hairlines, three status colours, a hover band, a code background) authored for light and dark and mapped onto shadcn's variables, so `Badge`, `Card`, `Table` and `Skeleton` take the look without per-component overrides; system font stacks; every state a dot plus its word; negatives in parentheses; `table-layout: fixed` with explicit column widths. Rejected: Radix `Tabs` for the sub navigation (it is navigation between routes, so it is links), a client-side expand for hold details (the open row is a query parameter, so the page stays a server component and the URL is shareable). Why: the brief is a data pipeline, not a web app; the console exists to make the demo legible, and every piece of client JavaScript would have been weight without a reader.

## D17. The theme switch is the console's one client island, and the cookie is its only store

Taken after the first demo. A switch that reloads the page reads as broken. Chosen: a 30-line client component; a click flips the class on `<html>` at once and writes the `theme` cookie, and the server reads that cookie on the next request so the class is already on `<html>` before paint. Two links are rendered and CSS shows the one for the theme that is not on screen, so there is no React state and nothing to hydrate wrongly; without JavaScript the links fall back to `/theme`, which sets the same cookie and redirects. Rejected: `localStorage`. It is invisible to the server, so every page would render in the default theme and correct itself after hydration (a flash on each navigation) unless an inline script ran before paint; the cookie gives persistence and a correct first paint with one mechanism.

## D18. `freeze` is the second late-arrivals policy, decided at staging time by a posting day

Chosen: every staged record gets a `posting_day` (migration 003). Under `restate` it is the record's own day. Under `freeze` it is the record's own day unless that day is already **closed for the source**, in which case it is the day the record arrived. A day is closed for a source when the delivery the manifest says should carry it has loaded before the current file. The marts group by `posting_day` and never look at the policy; the late record is listed in `mart.late_postings` with its own day and the day it was booked on. A refund whose order arrives later has its hold released under both policies, but under `freeze` its day is not rebuilt for that reason alone, so a closed day's channel split does not move either.
Rejected: closing a day when the marts have built it. The fixtures showed why: refund batches carry refunds for days weeks ahead (F4), so a day can have a refund-only row long before its orders' delivery arrives; treating that as "reported" would send the on-time orders to the arrival day. Also rejected: applying the policy in the marts (two code paths to keep equal) and a per-record flag with no audit view.
What the guarantee is, exactly: under `freeze` no number on a closed day changes. Open days still fill in as their deliveries arrive (30 and 31 Jan gain their orders when batch 5 lands), and those changes are recorded like any other; the arrival day accumulates the late records. Freeze needs `expected_deliveries`: a source with no manifest entries never closes a day and behaves as `restate`. The test replays batches 1 to 4 and then 5 under `freeze`: zero restatements on any day up to 29 Jan, the 24 late email events on today's row, totals identical to `restate`.
