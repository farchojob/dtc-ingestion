  ...
  return (
    <Panel className="px-[22px] py-[22px] md:px-9 md:pb-[34px] md:pt-8">
      ...
      <div className="grid grid-cols-2 gap-x-[18px] gap-y-6 md:grid-cols-4 md:gap-y-0">
        <Stat href={`/${tenant.id}/deliveries`} label="Deliveries" value={`${o.loaded} / ${o.expected}`} bad={o.missing > 0}
          note={o.missing ? `${o.missing} batch${o.missing > 1 ? "es" : ""} never arrived` : "all arrived"} />
      ...
      <Eyebrow>deliveries · {o.matrix.length} sources × {o.matrix[0]?.cells.length ?? 0} batches</Eyebrow>
...
export function MiniMatrix({ matrix, tenantId }: { matrix: MatrixRow[]; tenantId: string }) {
  return (
    <div className="mt-[14px] grid grid-cols-[112px_repeat(5,minmax(0,1fr))] items-center gap-x-2 gap-y-[10px]">
      {matrix.map((row) => (
        <MatrixLine key={row.source} row={row} tenantId={tenantId} />
      ))}
    </div>
  );
}

function MatrixLine({ row, tenantId }: { row: MatrixRow; tenantId: string }) {
  return (
    <>
      <span className="whitespace-nowrap font-mono text-[12px] text-ink-muted">{row.source}</span>
      {row.cells.map((c) => (
        <Link key={c.batch} href={`/${tenantId}/deliveries`} title={`${row.source} batch ${c.batch}: ${c.status}`}
